...
class SettingsWindow(BaseWindow):
    ...
    def init_settings_ui(self):
        ...
        self.use_api_checkbox = self.findChild(
            QCheckBox, 'model_options_use_api_input',
        )
        if self.use_api_checkbox:
            self.use_api_checkbox.stateChanged.connect(
                lambda: self.toggle_api_local_options(
                    self.use_api_checkbox.isChecked()
                )
            )
    ...
    def create_buttons(self):
        ...
        reset_button.setFont(font(10))
        reset_button.clicked.connect(self.reset_settings)
        self.main_layout.addWidget(reset_button)

        save_button = QPushButton('Save')
        save_button.setFont(font(10))
        save_button.setStyleSheet(
            'QPushButton { background:#2d5a8a; border:1px solid #4a9eff;'
            '              border-radius:6px; color:#e0e0e0; padding:6px 16px; }'
    ...
    def add_setting_widget(self, layout, key, meta, category, sub_category=None):
        ...
        widget_name = (
            f"{category}_{sub_category}_{key}_input"
            if sub_category
            else f"{category}_{key}_input"
        )
        label_name = (
            f"{category}_{sub_category}_{key}_label"
            if sub_category
            else f"{category}_{key}_label"
        )
        help_name = (
            f"{category}_{sub_category}_{key}_help"
            if sub_category
            else f"{category}_{key}_help"
        )
                ...
                if isinstance(sub_settings, dict) and 'value' in sub_settings:
                    ...
                    for key, meta in sub_settings.items():
                        widget = self.findChild(
                            QWidget, f"{category}_{sub_category}_{key}_input",
                        )
                        if widget:
